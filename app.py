        ...
        for venue in venues:
            if grouped_venue.city == venue.city:
                venue_data.append({
                    "id": venue.id,
                    "name": venue.name,
                    "num_upcoming_shows": len([Show.query.filter(venue_id == venue.id).filter(Show.start_time > today)])
                })
        real_data.append({
...
def show_venue(venue_id):
    ...
    # db.session.query(Show).join(Venue).filter(Show.venue_id == venue_id).filter(Show.start_time < today).all()   
    past_shows_query = Show.query.join('venues').filter(Show.venue_id == venue_id).filter(Show.start_time < today).all() 
    past_shows = []
    past_shows_count = 0
    ...
    # db.session.query(Show).join(Venue).filter(Show.venue_id == venue_id).filter(Show.start_time > today).all()   
    upcoming_shows_query = Show.query.join('venues').filter(Show.venue_id == venue_id).filter(Show.start_time > today).all()
    upcoming_shows = []
    upcoming_shows_count = 0
    ...
    real_data.upcoming_shows_count = upcoming_shows_count
    real_data.upcoming_shows = upcoming_shows
    real_data.past_shows_count = past_shows_count
...
def show_artist(artist_id):
    ...
    # db.session.query(Show).join(Artist).filter(Show.artist_id == artist_id).filter(Show.start_time < today).all() 
    past_shows_query = Show.query.join('artists').filter(Show.artist_id == artist_id).filter(Show.start_time < today).all()
    past_shows = []
    past_shows_count = 0
    for show in past_shows_query:
        past_shows_count += 1
    ...
    # db.session.query(Show).join(Artist).filter(Show.artist_id == artist_id).filter(Show.start_time > today).all() 
    upcoming_shows_query = Show.query.join('artists').filter(Show.artist_id == artist_id).filter(Show.start_time > today).all()   
    upcoming_shows = []
    upcoming_shows_count = 0
    for show in upcoming_shows_query:
        upcoming_shows_count += 1
